...
void setup() { //Define setup variables and pin confiurations
  ...
  attachInterrupt(digitalPinToInterrupt(CamshaftReferencePin), ISR1, RISING); //Reference must trigger on CHANGE to sniff sync window. Will later on in code be written to RISING after sync has been achieved
...
void loop() { //Program main loop
  while(syncAchieved == 1){//Start sequencing ignition outputs when sync has occured. Otherwise just wait until engine starts rotating.
    PORTB = fireOrder[cylinderCounter]; //Set the scheduled cylinder to fire
    
    //Measure rpm and disable/enable wasted spark if above the specified rpm
    if (halfEngineRev >= 70) { //Update RPM every 70 CR pulses, increase this value for better RPM resolution,decrease for lower accuracy but lower CPU usage
        rpm = 30000/(millis() - timeold)*halfEngineRev; //This formula calculates real rpm based on distributor pulses
        timeold = millis(); //Note that millis will overflow after 50!days of running engine nonstop, not a real issue?
        halfEngineRev = 0;  //Reset the counter
        if (rpm > 3000){  //set this to the desired above rpm that you want to switch to fully sequential ignition
          fireOrder[0] = B00000001; //Sequential ignition 
          fireOrder[1] = B00000100; 
          fireOrder[2] = B00001000; 
          fireOrder[3] = B00000010; 
      }
        else{ 
          fireOrder[0] = B00001001; //Wasted spark ignition
          fireOrder[1] = B00000110; 
          fireOrder[2] = B00001001; 
          fireOrder[3] = B00000110; 
   }
  }
 }
}
...
void ISR1() { //Keep constant track of CHANGE/(RISING after sync) state for reference pin
  ...
  if (syncAchieved == 1) { //If we have achieved sync, do only this part of the interrupt
    if(digitalRead(CamshaftReferencePin) == HIGH){ //This is an "unneeded if". It filters out spark EMI, incorrect pulses will be counted otherwise.
      ...
      halfEngineRev++; //we also increase variable indicating that the engine has performed one half crankshaft rotation
